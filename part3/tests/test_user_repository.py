...
class TestUserRepository(unittest.TestCase):
    ...
    def test_place_review_and_amenity_persist_with_sqlalchemy(self):
        with self.app.app_context():
            ...
            self.assertEqual(fetched_place.owner.id, owner.id)
            self.assertEqual(fetched_review.author.id, owner.id)
            self.assertEqual(fetched_review.place.id, place.id)
            self.assertEqual(fetched_place.amenities[0].id, amenity.id)
